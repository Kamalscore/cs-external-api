...
from flask_restplus.mask import Mask, apply as apply_mask
from flask_restplus.utils import unpack
from flask_restplus.fields import Wildcard
...
def custom_marshal(data, fields, envelope=None, skip_none=False, mask=None, ordered=False, output_key_enabled=False):
    ...
    """
    out, has_wildcards = _marshal(data, fields, envelope, skip_none, mask, ordered, output_key_enabled)

    if has_wildcards:
        # ugly local import to avoid dependency loop
        from custom_fields import Wildcard
        ...
        for dkey, val in fields.items():
            ...
            if isinstance(val, dict):
                value = custom_marshal(data, val, skip_none=skip_none, ordered=ordered)
            else:
                field = make(val)
...
def _marshal(data, fields, envelope=None, skip_none=False, mask=None, ordered=False, output_key_enabled=False):
    """Takes raw data (in the form of a dict, list, object) and a dict of
    fields to output and filters the data based on those fields.
    ...
    if isinstance(data, (list, tuple)):
        out = [custom_marshal(d, fields, skip_none=skip_none, ordered=ordered) for d in data]
        if envelope:
            out = OrderedDict([(envelope, out)]) if ordered else {envelope: out}
        return out, False
    ...
    def __format_field(key, val):
        ...
        key = field.output_key if output_key_enabled and field.output_key is not None else key
        return (key, value)

    items = (
        (k, custom_marshal(data, v, skip_none=skip_none, ordered=ordered))
    ...
    def __call__(self, f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if isinstance(resp, tuple):
                ...
                return (
                    custom_marshal(data, self.fields, self.envelope, self.skip_none, mask, self.ordered),
                    code,
                    headers
                )
            else:
                return custom_marshal(resp, self.fields, self.envelope, self.skip_none, mask, self.ordered)
        return wrapper
